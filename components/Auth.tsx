...
export const Auth: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Clear any existing session to ensure clean login state
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isForgotPassword) {
        ...
        // If they just paid, renew their subscription automatically
        if (hasPaid) {
          try {
            await supabase.rpc('renew_subscription', { duration_days: 30 });
            console.log("Subscription renewed/activated.");
          } catch (renewError) {
            console.error("Renewal failed", renewError);
          }
        }

        // Small delay to ensure Auth State propagates to App.tsx
        setTimeout(() => {
          navigate('/dashboard');
        }, 100);
    ...
    <div className="h-[100dvh] w-full bg-dark-900 overflow-y-auto relative">
      ...
      <div className="min-h-full flex flex-col items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-md bg-dark-800 border border-dark-700 rounded-2xl p-6 sm:p-8 shadow-2xl my-auto mt-16 sm:mt-auto">
          ...
          <div className="mt-8 pt-4 border-t border-dark-700 text-center">
            <button
              type="button"
              onClick={async () => {
                ...
                else alert(`Connection SUCCESSFUL (${duration}ms). DB is reachable.`);
              }}
              className="text-xs text-brand-500/50 hover:text-brand-500 transition-colors"
            >

...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  const handleShapeClick = (_e: any, id: string) => {
  ...
  const handleTextDblClick = (_e: any, id: string) => {
    if (currentTool === ToolType.SELECT) {
            ...
            {shapes.filter(s => s.page === currentPage).map((shape) => {
              ...
              if (shape.type === ToolType.LINE || shape.type === ToolType.PEN) {
                return <Line
                  ...
                  perfectDrawEnabled={false}
                  listening={currentTool === ToolType.SELECT}
                  hitStrokeWidth={currentTool === ToolType.SELECT ? 20 : 0}
                />;
              }
              ...
              if (shape.type === ToolType.MEASURE) {
                ...
                return (
                  <Group key={shape.id} {...commonProps} x={0} y={0} opacity={shape.opacity}>

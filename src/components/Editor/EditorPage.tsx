...
import { Save, ChevronLeft, ChevronRight, ZoomIn, ZoomOut, Maximize, AlertCircle, PanelRightClose, PanelRightOpen, Download, X, Undo, Redo, CheckCircle2, Users, MessageSquare } from 'lucide-react';
import { Button } from '../ui/Button';
import { TeamModal } from '../Project/TeamModal';
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { 
        ...
        // Sync project data including team members
        const updatedProject = payload.new as Project;
        setProject(prev => prev ? ({ ...prev, ...updatedProject }) : updatedProject);
        
        // Update shapes if they differ
        if (updatedProject.annotations && JSON.stringify(updatedProject.annotations) !== JSON.stringify(shapes)) {
          setShapes(updatedProject.annotations);
          lastSavedShapes.current = updatedProject.annotations;
        }
      })
      .subscribe();
    ...
    <div className="flex flex-col h-[100dvh] bg-dark-900 text-white overflow-hidden relative">
      ...
      <div className="h-14 border-b border-dark-700 bg-dark-800 flex items-center justify-between px-2 md:px-4 z-20 shrink-0 gap-2">
        ...
        <div className="hidden lg:flex items-center gap-6 absolute left-1/2 -translate-x-1/2">
          <div className="flex items-center gap-3 bg-dark-700/50 rounded-xl p-1.5 border border-dark-600">
            <Button variant="ghost" size="sm" disabled={currentPage <= 1} onClick={() => renderPage(pdfDoc, currentPage - 1)} className="h-9 w-9 p-0 hover:bg-dark-600">
              <ChevronLeft size={20} />
            </Button>
            <span className="text-sm min-w-[100px] text-center font-bold tracking-tight text-brand-400">Page {currentPage} / {pdfDoc?.numPages || 1}</span>
            <Button variant="ghost" size="sm" disabled={!pdfDoc || currentPage >= pdfDoc.numPages} onClick={() => renderPage(pdfDoc, currentPage + 1)} className="h-9 w-9 p-0 hover:bg-dark-600">
              <ChevronRight size={20} />
            </Button>
          </div>

          <div className="flex items-center gap-1.5 bg-dark-700/50 rounded-xl p-1.5 border border-dark-600">
            <button className="h-9 w-9 hover:bg-dark-600 rounded-lg flex items-center justify-center text-gray-400 hover:text-white transition-colors" onClick={() => setStageScale(s => Math.max(0.1, s - 0.1))}><ZoomOut size={18} /></button>
            <span className="text-sm w-14 text-center font-bold">{Math.round(stageScale * 100)}%</span>
            <button className="h-9 w-9 hover:bg-dark-600 rounded-lg flex items-center justify-center text-gray-400 hover:text-white transition-colors" onClick={() => setStageScale(s => s + 0.1)}><ZoomIn size={18} /></button>
            <div className="w-px h-6 bg-dark-600 mx-1"></div>
            <button className="h-9 w-9 hover:bg-dark-600 rounded-lg flex items-center justify-center text-brand-400 hover:text-brand-300 transition-colors" onClick={() => setFitTrigger(n => n + 1)} title="Fit to screen"><Maximize size={18} /></button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 md:gap-3 shrink-0">
          ...
          <Button variant="ghost" size="sm" onClick={() => setIsTeamModalOpen(true)} className="px-2 h-8 text-gray-400 hover:text-white">
            <Users size={16} />
          ...
          <Button 
            ...
            onClick={() => {
              ...
              if (!showChat && window.innerWidth < 1024) setShowProperties(false);
            }} 
            className={`px-2 h-8 transition-colors ${showChat ? 'text-brand-400 bg-brand-400/10' : 'text-gray-400 hover:text-white'}`}
          >
          ...
          <button
            onClick={() => {
              ...
              if (!showProperties && window.innerWidth < 1024) setShowChat(false);
            }}
      ...
      <div className="flex flex-1 overflow-hidden relative min-h-0">
        ...
        <div className="flex-1 flex flex-col relative overflow-hidden bg-neutral-900/90 min-h-0">
          ...
          <div className="lg:hidden shrink-0 h-14 bg-dark-800 border-t border-dark-700 flex items-center justify-between px-4 z-30">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold w-18 text-center text-brand-400">
        ...
        {/* Collapsible Panels - Absolute overlay on mobile, static on desktop */}
        <div className="flex h-full shrink-0">
          {showChat && (
            <div className={`absolute right-0 top-0 bottom-0 z-50 md:relative md:z-0 h-full shadow-2xl md:shadow-none w-[85vw] max-w-sm md:w-80 shrink-0 flex flex-col animate-in slide-in-from-right-10 duration-200`}>
                <ChatPanel 
                  projectId={id!} 

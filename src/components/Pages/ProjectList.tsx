...
import { Plus, FileText, Clock, UploadCloud, Trash2, Search, CheckCircle2, PencilRuler, AlertTriangle, X, ArrowLeft, Users } from 'lucide-react';
...
export const ProjectList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Listen for changes to ANY project this user is involved in
    const channel = supabase
      ...
      .on('postgres_changes', { 
        ...
        // If a project is updated, refresh the list
        fetchProjects();
      })
      .subscribe();
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="flex items-center gap-2 relative z-10" onClick={(e) => e.stopPropagation()}>
            ...
            <button 
                ...
                className="p-1.5 text-gray-500 hover:text-brand-500 hover:bg-brand-500/10 rounded-lg transition-colors"
      ...
      <div className="flex items-center text-xs text-gray-500 mt-4 gap-4">
        ...
        {project.team_members && project.team_members.length > 0 && (
          <span className="flex items-center gap-1 bg-dark-700 px-1.5 py-0.5 rounded text-brand-400">
            <Users size={12} /> {project.team_members.length} member{project.team_members.length !== 1 ? 's' : ''}
          </span>
        )}
      </div>
    </div>
      ...
      {isTeamModalOpen && projectForTeam && (
        <TeamModal 
          ...
          onUpdate={(updatedProject) => {
            ...
            setProjectForTeam(updatedProject);
          }}
        />
      )}
    </div>
